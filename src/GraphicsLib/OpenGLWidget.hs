...
module GraphicsLib.OpenGLWidget (
  openGLWidget
) where
...
openGLWidget color = defaultWidgetNode (WidgetType "openGLWidget") widget where
...
createShaderProgram = do
  ...
  vertexShader <- compileShader GL_VERTEX_SHADER "assets/opengl/shaders/vert.glsl"
  fragmentShader <- compileShader GL_FRAGMENT_SHADER "assets/opengl/shaders/frag.glsl"
